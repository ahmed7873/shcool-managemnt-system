    ...
    <div class="col-xl-12 mb-30">
        <div class="card card-statistics h-100">
            <div class="card-body">
                ...
                @endif
                <a href="{{ route('get_sections_students') }}" class="btn btn-success m-3">جلب طلاب</a>
                <form action="{{ route('unreleatoin_section_students') }}" method="post">
                    @csrf
                    <div class="table-responsive">
                        <table id="datatable" class="table  table-hover table-sm table-bordered p-0"
                            ...
                            <tbody>
                                ...
                                @foreach ($sectionStudents as $sectionStudent)
                                    <tr>
                                        <td><input type="checkbox" name="students[]" value="{{ $sectionStudent->id }}"
                                                class="box1"></td>
                                        <td>
                                            {{ $sectionStudent->NoKnow }}
                                        </td>
                                        <td>
                                            {{ $sectionStudent->name }}
                                        </td>
                        ...
                        <button type="submit" class="btn btn-danger my-3" style="width: 100%">حذف</button>
                    </div>
                </form>
            </div>
        </div>
