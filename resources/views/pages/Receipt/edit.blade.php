            ...
            <div class="card-body">
                ...
                <form action="{{ route('receipt_students.update', 'test') }}" method="post" autocomplete="off">
                    ...
                    <div class="row">
                        ...
                        <div class="col">
                            <label for="Name_en" class="mr-sm-2">لرسوم</label>
                            <div class="form-group">
                                <select class="fancyselect" name="invoice_id" required>
                                    <option value="">-- اختار من القائمة --</option>
                                    @foreach ($invices as $invice)
                                        <option value="{{ $invice->id }}"
                                            {{ $invice->id == $receipt_student->fee_invoice_id ? 'selected' : '' }}>
                                            {{ $invice->fees->title }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
